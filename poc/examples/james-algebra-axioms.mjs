import { Dot, makeEulerTree, treeFromEulerTree } from "tree-term-rewriting/src/tree";
...
const container = (boundary, child) => ({
...
const cloneTreeWithFreshIndices = (form) => {
  const state = { index: 1 };
  const walk = (node) => ({
    index: state.index++,
    value: node.label,
...
const printGraphLink = (title, trees) => {
