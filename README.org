...
** 4. API Sketch
* Current Gameplan
This demo has been a /hit/! But right now we gotta fix a couple of the demo examples.
...
collect the two frames ([oo] [o])s under the shared context [oo]
([oo] <[ ([oo] [oo]) ]>)

clarify
([oo] <[oo] [oo]>)

split because <> is not /really/ a boundary
([oo] <[oo]> <[oo]>)
...
** Fix: 4^2
I manually fixed it - one of the steps "Flatten to two copies of the base" had duplicate square containers.
** Fix: sqrt(2)
Needs a total overhaul. Scrap the existing example, then incorporate this:
